      ...
      <nav className="flex  items-center justify-between border-b-[1px] border-b-richBlack-700 bg-richBlack-800 px-5 py-3 pl-12 text-richBlack-25">
        ...
        <div className=" flex items-center pr-5 justify-between md:hidden">
          {menu && (
            <div>
              <ul
                className="flex z-5 gap-12 px-10 bg-black text-white basis-full absolute 
              w-screen p-5 top-0 left-0 right-0 bg-black/20 transition-all duration-500 cursor-pointer "
              >
                {NavbarLinks.map(
                  (link, idx) =>
                    link.title !== "Catalog" && (
          ...
          <IoReorderThreeOutline
            className={`text-4xl font-bold text-white z-10 md:hidden block cursor-pointer absolute 
              ${menu ? "hidden" : "right-6"}
            }
            `}
            onClick={openMenu}
        ...
        <div className="flex items-center gap-4 text-richBlack-200">
          ...
          {!user ? (
            <>
              <Link
                className="hidden rounded-lg px-4 py-1 ring-2 ring-blue-500 md:block"
                to={"/login"}
              >
                Log in
              </Link>
              ...
              {/* //for mobile width */}
              <Link
                className={`rounded-lg p-2 ring-2 ring-blue-500 block md:hidden absolute right-20 ${
                  menu && "hidden"
                }`}
                to={"/login"}
              >
  ...
  return (
    ...
    >
      <img
        ...
        width={30}
      />
      <BiCaretDown className="hidden sm:block" />
      <div
